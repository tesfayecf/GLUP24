...
datasets = [559, 563, 570, 575, 588, 591]
...
def train_all(model_name: str, model_version: int, **parameters) -> float:
    ...
    run_name = f"{model_name}_{model_version}-ALL-{prediction_}-[{timestamp}]"
    ...
    try:
        ...
        experiment_name = f"{experiment_name_}-ALL-{prediction_}"
    ...
    try:
        ...
        for dataset in datasets:
            train_val_dataset = pd.read_csv(f'{datasets_dir_}/{dataset}/{dataset}_train.csv', sep=';',encoding = 'unicode_escape', names=columns)
            train_val_dataset = process_data(train_val_dataset, scale_data=scale_data, select_features=select_features, use_differences=use_differences, prediction_time=prediction_time)
            train_val_data = pd.concat([train_val_data, train_val_dataset])
